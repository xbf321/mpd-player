    ...
    <ol className={clsx(["mb-3 flex items-center w-full p-3 space-x-2 text-sm font-medium text-center text-gray-500 bg-white border border-gray-200 rounded-lg shadow-xs dark:text-gray-400 sm:text-base dark:bg-gray-800 dark:border-gray-700 sm:p-4 sm:space-x-4 rtl:space-x-reverse", className])}>
      {items.map((item, index) => {
        const rootClassName = clsx({
          ...
          'text-blue-600 dark:text-blue-500': index <= current,
        });
        const cycleClassName = clsx({
          'flex items-center justify-center w-5 h-5 me-2 text-xs border  rounded-full shrink-0':
            true,
          'border-blue-600 dark:border-blue-500': index <= current,
        });
        return (
          <li key={index} className={rootClassName}>
            {item.status === StepperStatus.DONE ? (
              ...
              <span className={cycleClassName}>{index + 1}</span>
            )}

            {item.status === StepperStatus.CHECKING && (
              <svg
                aria-hidden="true"
                className="w-3.5 h-3.5 ml-1 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                viewBox="0 0 100 101"
                fill="none"
              >
                <path
